    ...
    if (!exists) {
      return knex.schema.createTable(table, function (t) {
        ...
        _.each(keys, function (k) {
          ...
          if (tableK.hasOwnProperty('created_at')) {
            column.dateTime();
          }

          if (tableK.hasOwnProperty('updated_at')) {
            column.dateTime();
          }
  ...
  if (c.user_id && !c.relation && !c.lookup) {
    ...
    new _model(c.where).fetch({ withRelated: c.relation })
      .then(function (m) {
        ...
        new model(c.body).save()
          .then(function (n) {
            if (c.geography) { c.geography(knex); }
            return done(null, n);
          })
          .catch(function (r) { return done(r); })
      })
...
DB.prototype.update = function (c, done) {
  ...
  if (c.user_id && c.relation) {
    new model(c.where).fetch({ withRelated: c.relation })
      .then(function (m) {
        if (!c.isAuthenticated(m)) return done(new Error());
