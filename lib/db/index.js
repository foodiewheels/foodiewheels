...
DB.prototype.create = function (c, done) {
  ...
  if (c.user_id) {
    lookup = this.getTable('truck');
    new lookup(c.where).fetch({withRelated: ['user']})
      .then(function (a) {
        if (a.relations.user.get('users_id') !== c.user_id) {
          return done(new Error());
        }
        c.body.truck_id = a.get('truck_id');
        new model(c.body).save()
          .then(function (o) { return done(null, o); })
          .catch(function (e) { return done(e); })
      })
      .catch(function (e) {done(new Error());});
  }
  else {

    function successNew (m) {
